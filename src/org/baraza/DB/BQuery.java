...
public class BQuery {
	...
	public void buildQuery(BDB db, BElement view, String wheresql, String orderby) {
		...
		for(BElement el : view.getElements()) {
			if(el.getAttribute("editkey") != null) colNames = addField(colNames, el.getAttribute("editkey"));
			else if(el.getAttribute("weblink") != null) colNames = addField(colNames, el.getAttribute("weblink"));
		}
	...
	public void filterBuild(String wheresql, String orderby) {
		...
		if(view != null) {
			...
			if(view.getAttribute("nouser") != null) {
				String userFilter = "(" + view.getAttribute("nouser") + " != '" + user.getUserID() + "')";
				if(wheresql == null) wheresql = "\nWHERE " + userFilter;
				else wheresql += " AND " + userFilter;
			}
			if(view.getAttribute("oruser") != null) {
				String userFilter = "((" + view.getAttribute("oruser") + " = '" + user.getUserID() + "')"
					+ " OR (" + view.getAttribute("oruser") + " is null))";
				if(wheresql == null) wheresql = "\nWHERE " + userFilter;
				else wheresql += " AND " + userFilter;
			}
			...
			if((view.getAttribute("filter.departments") != null) && (userOrg != null)) {
				if(user.getEntityDepartments() != null) {
					String fdWhere = view.getAttribute("filter.departments") 
						+ " IN (" + user.getEntityDepartments() + ")";
						
					if(wheresql == null) wheresql = "\nWHERE " + fdWhere;
					else wheresql += " AND " + fdWhere;
				}
			}
			
			if((view.getAttribute("filter.deployments") != null) && (userOrg != null)) {
				if(user.getEntityDeployments() != null) {
					String fdWhere = view.getAttribute("filter.deployments") 
						+ " IN (" + user.getEntityDeployments() + ")";
	...
	public boolean recEdit() {
		...
		if(!isAddNew) { 
			...
			if(user == null) changeTrack = "{userid:0";
			else changeTrack = "{userid:" + user.getUserID();
		}
		return isEdit;
	}
...
//System.out.println("BASE 1040 : " + el.getValue() + " : " + dataValue);
			...
			if(el.getAttribute("enabled","true").equals("false")) {
				...
				if(el.getAttribute("ischar") == null) {
					...
					else saveMsg += updateField(el.getValue(), dataValue);
				} else {
						...
						if(!fldName.trim().equals("")) {
							if (el.getName().equals("CHECKBOX")) {
								...
								if(javaCall.equals("password")) {
									newRow.addElement(BCipher.password(rs.getString(fldName)));
								}
							} else {
				...
				if(view == null) {
					...
					for(BElement el : view.getElements()) {
						if(!el.getValue().equals("")) {
							if(el.getAttribute("calendar", "false").equals("true")) forCal = true;
							else forCal = false;
							
							String cd = formatData(el);
							mystr.append("<td>");
							if(forCal) mystr.append("<div class='fc-event' key_field_id='" + rs.getString(keyField) + "'>");
							if(trim && (cd.length()>25)) mystr.append(cd.substring(0, 24));
							else mystr.append(cd);
							if(forCal) mystr.append("</div>");
							mystr.append("</td>");
						}
					}
				}
	...
	public JSONArray getJSON() {
		...
		try {
			...
			while (rs.next()) {
				...
				if(view == null) {
					for(int column = 0; column < titles.size(); column++) {
						String cValue = rs.getString(column + 1);
						if((titles.get(column) != null) && (cValue != null)) {
							myJo.put(titles.get(column), rs.getString(column + 1));
						}
					}
				} else {
					for(BElement el : view.getElements()) {
						if(!el.getValue().equals(""))
							myJo.put(el.getValue(), formatData(el));
					}
					if(view.getAttribute("no.keyfield", "false").equals("false"))
						myJo.put("keyfield", getKeyField());
				}
				myJa.put(myJo);
				
				i++;
				if((tableLimit > 0) && (tableLimit <= i)) break;
			}           	
 		} catch (SQLException ex) {
			log.severe("Field read data error : " + ex);
		}
...
//System.out.println("BASE JSON : " + jsTb.toString());
	...
	public String getRowJSONString() {
		return getRowJSON().toString();
	}

	public String getHtmlFields(String inputType) {
		String htmlData = "";
		try {
			if(view == null) {
				for(int column = 0; column < titles.size(); column++) {
					String dataValue = rs.getString(column + 1);
					if(dataValue != null) {
						htmlData += "<input type=\"" + inputType + "\" name=\"" + titles.get(column) + "\" value=\""
							+ dataValue + "\"/>\n";
					}
				}
			} else {
				for(BElement el : view.getElements()) {
					if(!el.getValue().equals("")) {
						String dataValue = rs.getString(el.getValue());
						if(dataValue != null) {
							htmlData += "<input type=\"" + inputType + "\" name=\"" + el.getValue() + "\" value=\""
								+ dataValue + "\"/>\n";
						}
					}
				}
	...
	public void importData(Vector<Vector<Object>> newData, Map<String, String> inParams)  {
		for(int j = 0; j<newData.size(); j++) {
			...
			if(!keyFieldData.contains(newRow.get(0).toString())) {
				...
				String linkData = inParams.get("data");
				if((view != null) && (linkData != null)){
					if(view.getAttribute("linkfield") != null) {
						updateField(view.getAttribute("linkfield"), linkData);
					}
				}
				
				if((view.getAttribute("user") != null) && (user != null)) {
					updateField(view.getAttribute("user"), user.getUserID());
				}
	...
	public String formatData(BElement el) {
		...
		try {
			if(rs.getString(el.getValue()) == null) {
				if(el.getAttribute("default") == null) response = "";
				else response = el.getAttribute("default");
			} else if(format == null) {
			...
			} else if(format.equals("truefalse")) {
				if(rs.getBoolean(el.getValue())) response = "true";
				else response = "false";
			...
			if(el.getAttribute("bold") != null) response = "<b>" + response + "</b>";
			if(el.getAttribute("italics") != null) response = "<i>" + response + "</i>";
		} catch(SQLException ex) {
    ...
    public String getCsvValueAt(int aRow, int aColumn) {
		...
		if(view != null) {
			if(view.getAttribute("csv.quote", "true").equals("false")) hasQuote = false;
		}

		String mystr = "";
		if(myobj!=null) {
			if(myclass==String.class) {
				if(hasQuote && myobj.toString().startsWith("0")) mystr = "\"'" + myobj.toString() + "\"";
				else mystr = "\"" + myobj.toString() + "\"";
				
				if(mystr.matches(".*\\<[^>]+>.*")) {
					mystr = BTextFormat.htmlToText(mystr);
				}
			}
			else mystr = myobj.toString();
		}
	...
	public String getXmlData(String ifNull) {
		...
		boolean uCase = false;
		boolean lCase = false;
		if(view.getAttribute("ucase", "false").equals("true")) uCase = true;
		if(view.getAttribute("lcase", "false").equals("true")) lCase = true;
