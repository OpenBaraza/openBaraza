...
public class BWebBody extends BQuery {
	...
	public String getGrid(List<String> viewKeys, List<String> viewData, boolean addJSc, String viewKey, boolean sfield) {
		...
		if(view.getName().equals("FORMVIEW")) {
			myhtml.append("<div class='table-scrollable table-responsive table-bordered viewform'>\n");
			...
			for(BElement el : view.getElements()) {
				...
				if(el.getName().equals("BROWSER")) {
					...
					colWidths += ", title: '" +  fieldTitle + "', sortable: true, filterable: true}";
				}
			}
			boolean hasGo = false;
			...
			while (rs.next()) {
				...
				for(BElement el : view.getElements()) {
					...
					if(!el.getValue().equals(""))  {
						...
						if (el.getName().equals("COLFIELD")) {
							...
							if(myAction == null) myAction = fieldTitle;
							...
							String mydv = "\n<td><a href='#";
							if(el.getAttribute("weblink") != null) mydv = "\n<td><a href='" + rs.getString(el.getAttribute("weblink"));
							else if(el.getAttribute("path") != null) mydv = "\n<td><a href='" + el.getAttribute("path");
							...
							String mydv = cellData;
							if(sk != null) {
								mydv = "\n<td><a href='?view=" + viewKey + ":" + sk + "&data=" + rs.getString(keyField) + "'>";
								mydv += cellData + "</a>";
							} else {
								mydv = "\n<td>";
							}
				...
				if(view.getName().equals("FILES")) {
					if(view.getAttribute("edit", "true").equals("true")) {
						...
						myhtml.append(" target='_parent'>Delete</a></td>");
					}
    ...
    public String getForm(boolean isNew, String formLinkData, HttpServletRequest request) {
		...
		for(BElement el : view.getElements()) {
			if(el.getAttribute("tab") != null) {				
				if(!tab.equals(el.getAttribute("tab"))) {
					...
					tabs.append("<a href='#" + tab.replace(" ", "") + "' data-toggle='tab'>" + tabName + " </a></li>\n");
				}
				hasTabs = true;
			}
		}
		...
		tab = "";
     	for(BElement el : view.getElements()) {
			...
			if(elName.equals("USERFIELD") || elName.equals("USERNAME") || elName.equals("REMOTEIP") || elName.equals("DEFAULT")) forForm = false;
			if(elName.equals("FUNCTION") || elName.equals("FORMVIEW")) forForm = false;
	...
	public String getField(BElement el, String formLinkData, boolean eof, Integer formCols) {
		...
		String defaultValue = db.getDefaultValue(el);
		
		String classForm = view.getAttribute("class.form", "col-md-6");
		...
		response.append("		<div class='" + classField + "'>\n");
		...
		if(el.getName().equals("HTML")) {
			...
			if(el.getAttribute("type", "2").equals("3")) dateFormatter = new SimpleDateFormat("HH:mm");
			if(eof) {
				if(getString(el.getValue()) != null) {
				...
				response.append(" value='" + dateFormatter.format(new Date()) + "'");
	...
	public String getComboBox(BElement el, String formLinkData, boolean eof, String fieldValue, String defaultValue, String jsFunction) {
		...
		for(String attributeName : el.getAttributeNames()) {		// select.detail
			if(attributeName.startsWith("select.")) response.append(attributeName.replace(".", "-"));
		}
		...
		} else if(el.getAttribute("nouser") != null) {
			String userFilter = "(" + el.getAttribute("nouser") + " != '" + db.getUserID() + "')";
			if(cmbWhereSql == null) cmbWhereSql = userFilter;
			else cmbWhereSql += " AND " + userFilter;
		}
		...
		while (cmbrs.moveNext()) {
			...
			for(String attributeName : el.getAttributeNames()) {
				if(attributeName.startsWith("select.")) {
					String sdc = cmbrs.getString(el.getAttribute(attributeName));
